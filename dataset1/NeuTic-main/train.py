...
def load_and_split_data():
    ...
    for label_id, label_name in id_to_label.items():
        count = class_counts[label_id] if label_id < len(class_counts) else 0
        weight = class_weights[label_id] if label_id < len(class_weights) else 1.0
        print(f"  {label_name} (ID:{label_id}): {count} 个样本, 权重: {weight:.4f}")
    print("=" * 60)
    ...
    return train_set, val_set, test_set, id_to_label, class_weights
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (X_batch, pos_batch, y_batch) in enumerate(loader):
        # 将数据移至设备
...
def main():
    ...
    print("="*60)
    print("项目名称：云平台上的加密 TLS 流量分类（6类别）")
    print("姓名：黄宇琳")
    print(f"当前系统时间：{current_time}")
    print("="*60)
    
    print("开始 NeuTic 模型训练与验证（6类别）")
    print(f"使用设备: {cfg.device}")
    print("="*60)

    # 1. 加载数据（现在返回5个值，包括class_weights）
    train_set, val_set, test_set, id_to_label, class_weights = load_and_split_data()
    ...
    print(f"分类类别数: {cfg.num_classes}")

    # 3. 定义损失函数和优化器（使用动态计算的类别权重）
    # 将权重移动到与模型相同的设备
    class_weights = class_weights.to(cfg.device)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.Adam(model.parameters(), lr=cfg.learning_rate)
    
    print("使用带权重的交叉熵损失函数处理类别不平衡")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("【6类别分类任务评估】")
    print("基础学习能力：测试准确率应显著高于随机猜测 (16.67%)。")
    print(f"你的结果: {test_acc:.2f}% {'✅' if test_acc > 20.0 else '⚠️'}")
    
    # 计算每个类别的准确率
    from sklearn.metrics import precision_recall_fscore_support
    precision, recall, fscore, support = precision_recall_fscore_support(
        test_labels, test_preds, labels=range(len(label_names))
    )
    
    print("\n【各类别详细性能】")
    for i, label in enumerate(label_names):
        print(f"  {label}: 精确率={precision[i]:.3f}, 召回率={recall[i]:.3f}, F1={fscore[i]:.3f}")
    ...
    results = {
        ...
        'class_weights': class_weights.cpu().numpy(),  # 保存使用的权重
        'precision_recall_fscore': (precision, recall, fscore)
    }
